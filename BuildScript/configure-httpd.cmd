...
echo(>> "%HTTPDINSTDIR%\conf\httpd.conf"
echo LoadModule dav_svn_module     C:/SVN-%VER%-%HTTPDVER%/svn-win32-%VER%/svn-win32-%VER%/bin/mod_dav_svn.so>> "%HTTPDINSTDIR%\conf\httpd.conf"
echo LoadModule authz_svn_module   C:/SVN-%VER%-%HTTPDVER%/svn-win32-%VER%/svn-win32-%VER%/bin/mod_authz_svn.so>> "%HTTPDINSTDIR%\conf\httpd.conf"
echo(>> "%HTTPDINSTDIR%\conf\httpd.conf"
echo ^<Location /svn-test-work/repositories^>>> "%HTTPDINSTDIR%\conf\httpd.conf"
echo  DAV svn>> "%HTTPDINSTDIR%\conf\httpd.conf"
echo  SVNParentPath C:/SVN-%VER%-%HTTPDVER%/src-%VER%/Release/subversion/tests/cmdline/svn-test-work/repositories>> "%HTTPDINSTDIR%\conf\httpd.conf"
echo  SVNListParentPath On>> "%HTTPDINSTDIR%\conf\httpd.conf"
echo  AuthzSVNAccessFile C:/SVN-%VER%-%HTTPDVER%/src-%VER%/Release/subversion/tests/cmdline/svn-test-work/authz>> "%HTTPDINSTDIR%\conf\httpd.conf"
...
echo  DAV svn>> "%HTTPDINSTDIR%\conf\httpd.conf"
echo  SVNPath C:/SVN-%VER%-%HTTPDVER%/src-%VER%/Release/subversion/tests/cmdline/svn-test-work/local_tmp/repos>> "%HTTPDINSTDIR%\conf\httpd.conf"
echo  AuthzSVNAccessFile C:/SVN-%VER%-%HTTPDVER%/src-%VER%/Release/subversion/tests/cmdline/svn-test-work/authz>> "%HTTPDINSTDIR%\conf\httpd.conf"
echo  AuthType Basic>> "%HTTPDINSTDIR%\conf\httpd.conf"
echo  AuthName "Subversion Repository">> "%HTTPDINSTDIR%\conf\httpd.conf"
echo  AuthUserFile "%HTTPDINSTDIR%/conf/users">> "%HTTPDINSTDIR%\conf\httpd.conf"
